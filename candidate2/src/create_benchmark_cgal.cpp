...
CurveID getRandomCurve(Query const& query)
{
...
{
	using QueryPair = std::pair<CurveID, distance_t>;
	...
	std::vector<std::size_t> ks = {0, 1, 5, 20, 50};//, 100, 1000};

	Query query(curve_directory);
	query.readCurveData(curve_data_file);
	query.getReady();

	std::ofstream file(out_prefix + ".txt");
	...
	for (std::size_t i = 0; i < ks.size(); ++i) {
		...
		for (CurveID curve_id = 0; curve_id < 100; ++curve_id) {
			Curve const& query_curve = query.getCurves()[curve_id];
			...
			if (distance_found) {
				query_pairs.emplace_back(curve_id, query_distance);
				std::cout << "." << std::flush;
			}
		...
		std::cout << "Verifying and exporting to file..." << "\n";
		query.setAlgorithm("naive");
		if (file.is_open()) {
			file << std::setprecision(20);
			for (auto const& query_pair: query_pairs) {
				query.run(query.getCurves()[query_pair.first], query_pair.second);
				assert(query.getResults()[0].curve_ids.size() == k+1);

				file << query_pair.first << " " << query_pair.second;
				for (auto result_id: query.getResults()[0].curve_ids) {
					file << " " << result_id;
				}
				file << "\n";
			}
