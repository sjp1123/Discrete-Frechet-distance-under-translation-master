...
struct Times
{
	...
	void startCheckCertificate(CertType type) { certcheck_start[type] = hrc::now(); }
	void startBuildOrthRangeSearch() { buildorthrange_start = hrc::now(); }
	void startFindNoTraversal() { findno_start = hrc::now(); }

	void stopCertifying() { certifyingcomp_sum += stop(certifyingcomp_start); }
	...
	void stopCheckCertificate(CertType type) { certcheck_sum[type] += stop(certcheck_start[type]); }
...
struct Times
{
	...
	void stopCheckCertificate(CertType type) {}
