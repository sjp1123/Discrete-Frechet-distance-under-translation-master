...
FrechetUnderTranslation::FrechetUnderTranslation(distance_t const epsilon)
	: FrechetUnderTranslation(epsilon, 40, 12)
{
}
	...
	while (!search_boxes.empty()) {
		...
		if (search_box.layer >= depth_limit) {
			if (arrangement_enabled) {
				...
				{ Point bmin = search_box.min, bmax = search_box.max; distance_t rad = min_dist;
				  n6_alg.setBox(N6Alg::BoundingBox{{bmin.x, bmin.y}, {bmax.x, bmax.y}});
				  n6_alg.setContainProvider([this, bmin, bmax, rad](Points& out) { collectContainCenters(bmin, bmax, rad, out); }); }
				// only calculate the actual distance of this box if it improves max
				if (n6_alg.lessThan(max, curve1, curve2)) {
					if (n6_range_fix()) {
						auto const v = n6_alg.calcDistanceRange(curve1, curve2, min_dist, max);
						if (v < max) { max = v; }
					}
					else {
	...
	while (!search_boxes.empty()) {
		...
		if (search_boxes.currentLayer() >= depth_limit) {
			...
			if (arrangement_enabled) {
				...
				N6Alg n6_alg(epsilon_sub);
				n6_alg.setCandidateCenters(cut_centers);
				{ Point bmin = search_box.min, bmax = search_box.max; distance_t rad = distance;
				  n6_alg.setBox(N6Alg::BoundingBox{{bmin.x, bmin.y}, {bmax.x, bmax.y}});
				  n6_alg.setContainProvider([this, bmin, bmax, rad](Points& out) { collectContainCenters(bmin, bmax, rad, out); }); }
...
// FIX: every disc of radius `radius` that contains the box [bmin, bmax].  Such a
// disc's centre is within `radius` of the box centre, so the kd-tree query is a
// plain ball query; the check keeps exactly the discs holding all four corners.
void FrechetUnderTranslation::collectContainCenters(Point bmin, Point bmax, distance_t radius, Points& out) const
{
	auto const r2 = radius * radius;
	auto contains = [&](Point const& c) {
		auto in = [&](distance_t x, distance_t y) { auto dx = x - c.x, dy = y - c.y; return dx*dx + dy*dy <= r2; };
		return in(bmin.x, bmin.y) && in(bmin.x, bmax.y) && in(bmax.x, bmin.y) && in(bmax.x, bmax.y);
	};
	if (radius <= 0) { return; }
	if (use_kd_tree) {
		Point const o{(bmin.x + bmax.x)/2., (bmin.y + bmax.y)/2.};
		kd_tree.search({o.x, o.y}, 0, radius, out, false, contains);
	}
	else {
		auto const& curve1 = *curve_pair[0];
		auto const& curve2 = *curve_pair[1];
		for (std::size_t i = 0; i < curve1.size(); ++i) {
			for (std::size_t j = 0; j < curve2.size(); ++j) {
				auto center = curve1[i] - curve2[j];
				if (contains(center)) { out.push_back(center); }
			}
		}
	}
}
