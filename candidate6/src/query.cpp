...
void Query::check_certificate(Certificate const& c, Times::CertType type) {
	...
	if (c.isValid()) {
		...
		global::times.startCheckCertificate(type);
		if (c.check()) {
			if (c.isYes()) {
		...
		global::times.stopCheckCertificate(type);
...
void Query::printDataStats(bool as_table) const
{
	...
	float sum_cone_changes = 0.;
	int min_cone_changes = 10000000.;
	int max_cone_changes = -1.;
	for (auto& curve: curve_data) {
		auto cone_changes = calcConeChanges(curve, 17.*M_PI/32.);
		sum_cone_changes += cone_changes;
		min_cone_changes = std::min(min_cone_changes, cone_changes);
		max_cone_changes = std::max(max_cone_changes, cone_changes);
	}
	...
	if (as_table) {
		...
		std::cout << "Min/Max/Avg cone changes: " << min_cone_changes << " / " << max_cone_changes << " / " << sum_cone_changes/curve_data.size() << "\n";
	}
}
