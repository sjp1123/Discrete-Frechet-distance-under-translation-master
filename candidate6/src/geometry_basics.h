...
{
public:
	static constexpr distance_t eps = 1e-12;
...
//
// Types for the discrete Fréchet distance
//

struct PInterval;
using PIntervals = std::vector<PInterval>;
using PIntervalsID = ID<PIntervals>;
using PIntervalID = std::size_t;
using PIntervalIDs = std::vector<PIntervalID>;

// The interval is including the end!
struct PInterval
{
	PointID begin;
	PointID end;
	...
	PInterval()
		: begin(std::numeric_limits<PointID::IDType>::max()),
		  end(std::numeric_limits<PointID::IDType>::lowest()) {}

	PInterval(PInterval const& other) = default;

	PInterval(PointID begin, PointID end)
		: begin(begin), end(end) {}
	
	bool operator<(PInterval const& other) const {
		return begin < other.begin || (begin == other.begin && end < other.end);
	}
