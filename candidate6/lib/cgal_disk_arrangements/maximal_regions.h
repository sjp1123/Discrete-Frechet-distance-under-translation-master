...
struct Stats {
	...
	// FIX (box-containing discs): enumerate_block() bookkeeping
	long long blk_calls = 0, blk_hull = 0, blk_drop = 0, blk_ns = 0;
	double max_excess = 0, max_r = 0;
	long long q1_calls = 0, q2_calls = 0, q2_exact = 0, wit_exact = 0;   // AUDIT: max(|w - c| - r) over emitted witnesses, largest r seen
};
...
// FIX: enumerate the inclusion-maximal X ⊆ `discs` such that X ∪ Block has a
// common point, where Block = discs of radius `r` centred at `block` (the convex
// hull vertices of the centres of every disc that contains the search box).
// Helly on the family X ∪ Block reduces this to the existing P2/P3 predicates:
//   single  i     : P2(i,a) ∀a,  P3(i,a,a') ∀a<a'
//   pair    i,j   : P2(i,j) ∧ P3(i,j,a) ∀a
//   triple  i,j,k : P3(i,j,k)
// (triples inside Block hold because every block disc contains the box).  The
// DP stays over subsets of `discs` only, so the 2^m term does not grow.  Witness
// = MEC centre of X's centres ∪ block, which lies in every block disc.
Result enumerate_block(Discs const& discs, std::vector<Point> const& block, double r,
                       Params const& params, Stats* stats);

// FIX (box family): enumerate the inclusion-maximal X ⊆ `discs` with
// ∩X ∩ box ≠ ∅.  Helly on X ∪ {box}: P3 on disc triples, Q1(i): D_i ∩ box ≠ ∅,
// Q2(i,j): D_i ∩ D_j ∩ box ≠ ∅.  In exact mode all three are filtered-exact
// (double filter + cpp_rational fallback, sqrt-free).  The witness is the box
// point minimising max_i |p − c_i|; it is certified to lie within
// r + witness_tol of every member (recomputed exactly and rounded if the double
// certificate fails), and it lies in the box, hence in every disc containing it.
