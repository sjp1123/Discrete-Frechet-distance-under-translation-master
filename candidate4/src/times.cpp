...
std::ostream& operator<<(std::ostream& out, Times const& times)
{
...
#ifdef CERTIFY
	out << std::setprecision(3) << std::fixed
	<< "   - certificate computation (unfiltered): " << times.certcomp_sum/1000000000. << "s\n"
	...
	<< "   - certificate check: " << (times.certcheck_sum[Times::COMPLETE]+times.certcheck_sum[Times::FILTER])/1000000000. << "s\n"
	<< "   \t* filtered: " << times.certcheck_sum[Times::FILTER]/1000000000. << "s\n"
	<< "   \t* unfiltered: " << times.certcheck_sum[Times::COMPLETE]/1000000000. << "s\n";
#endif 
